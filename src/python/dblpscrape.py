...
def ensure_conference_exists(cur: Cursor, conference_name: str, year: int, latitude: str, longitude: str) -> None:
    """
    ...
    try:
        ...
        cur.execute(
            sql, (conference_name, year, latitude, longitude),
        )
...
def insert_paper(cur: Cursor, paper: PaperDetails, conf_name: str, conf_year: int) -> bool:
    """
...
def main() -> None:
    ...
    parser.add_argument(
        "--url",
        required=True, 
        help="The full dblp.org URL to scrape."
    )
    ...
    try:
        ...
        ensure_conference_exists(cur, args.conference, args.year, args.latitude, args.longitude)
        conn.commit()
        ...
        print(f"Found {len(publication_ids)} publications. Starting to scrape for details")
